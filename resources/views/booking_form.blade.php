...
@section('content')
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 text-left relative">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-slate-100 h-72 md:h-96 rounded-2xl flex items-center justify-center border border-slate-200 shadow-sm relative overflow-hidden group">
                <div class="w-full h-full flex items-center justify-center">
                    @if($kos->foto)
                        <img id="sliderImage" src="{{ asset('foto-kos/' . $kos->foto) }}" class="w-full h-full object-cover object-center transition duration-300" alt="Foto Kos">
                    @else
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                ...
                <label class="flex items-center justify-between p-4 bg-slate-50 rounded-xl cursor-pointer border border-slate-200 hover:bg-slate-100 transition duration-200">
                    <div class="space-y-0.5">
                        <p class="text-sm font-bold text-slate-700">Tambah Armada / Jasa Pindahan</p>
                        <p class="text-xs text-slate-400 font-medium">Bantuan pengangkutan barang langsung ke lokasi kos baru oleh tim kami.</p>
                    </div>
            ...
            <div class="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-xl space-y-5 sticky top-6">
                ...
                <form id="formBookingReal">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Pilih Tanggal Check-in</label>
                    ...
                    <div class="space-y-1.5 pt-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Pilih Durasi Sewa</label>
                        <div class="grid grid-cols-2 gap-2" id="durasiContainer">
                            <button type="button" data-durasi="1"
                                class="durasi-btn py-2.5 border-2 border-blue-500 bg-blue-50 text-blue-600 font-bold rounded-xl text-xs tracking-wide">1 Bulan</button>
                            <button type="button" data-durasi="3"
                                class="durasi-btn py-2.5 border border-slate-200 text-slate-600 font-bold rounded-xl text-xs hover:border-blue-300">3 Bulan</button>
                            <button type="button" data-durasi="6"
                                class="durasi-btn py-2.5 border border-slate-200 text-slate-600 font-bold rounded-xl text-xs hover:border-blue-300">6 Bulan</button>
                            <button type="button" data-durasi="12"
                                class="durasi-btn py-2.5 border border-slate-200 text-slate-600 font-bold rounded-xl text-xs hover:border-blue-300">12 Bulan</button>
                        </div>
                    ...
                    <div class="border-t border-slate-100 pt-4 space-y-2 text-xs font-bold text-slate-600">
                        ...
                        <div class="border-t border-slate-100 pt-3 mt-2 flex justify-between items-center">
                            ...
                            <span id="txtTotalBayar" class="text-xl font-black text-blue-600" data-harga-asli="{{ $kos->harga }}">
    ...
    <div id="modalKonfirmasi" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-6 text-center space-y-4 border border-slate-100 animate-fade-in">
            <span class="text-3xl">⚠️</span>
            <h3 class="text-base font-black text-slate-800 tracking-wide uppercase">Konfirmasi Pembayaran</h3>
            <p class="text-xs text-slate-400 font-medium">Periksa kembali ringkasan rincian durasi dan total harga sewa kos Anda sebelum melanjutkan.</p>
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
                <button type="button" id="btnBatalKonfirmasi" class="py-2.5 bg-slate-100 text-slate-500 font-bold rounded-xl text-xs transition">Batal</button>
                <button type="button" id="btnKirimBookingFix" class="py-2.5 bg-blue-500 text-white font-bold rounded-xl text-xs shadow-md transition">Ya, Kirim</button>
            </div>
        </div>
    </div>

    <div id="modalPemilik" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl p-8 text-center space-y-6 border border-slate-100 animate-fade-in">
            <h3 class="text-base font-black text-slate-800 border-b pb-3 tracking-wide uppercase">Detail Booking</h3>

            <div class="space-y-4">
                <div class="w-16 h-16 bg-slate-100 mx-auto rounded-full flex items-center justify-center text-slate-400 text-[10px] font-black border border-slate-200">
                    OWNER
                </div>
                <div>
                    <h4 class="font-black text-slate-800 text-sm tracking-wide uppercase">Konfirmasi Pemilik Kos</h4>
                    <p class="text-xs text-slate-400 font-medium mt-1">Kamar ini memerlukan validasi manual dari pemilik kos.</p>
                </div>
                <p class="text-xs font-black text-slate-700 bg-slate-50 py-2.5 rounded-xl border border-slate-100 tracking-wider" id="txtNoWa">
                    Loading...
                </p>
            </div>
            ...
            <a href="{{ route('my.bookings') }}" class="block text-[11px] font-bold text-slate-400 hover:text-slate-600 transition pt-1">
